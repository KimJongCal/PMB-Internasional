<?php
defined('BASEPATH') OR exit('No direct script access allowed');
class Model_kelurahan extends CI_Model {

    public function create($data){
        if($this->db->insert('tbl_setting_kelurahan',$data)){
            return TRUE;
        }else{
            return FALSE;
        }
    }

    public function read(){
        return $this->db->get('tbl_setting_kelurahan');
    }

    public function update($id,$data){
        $this->db->where('id_kelurahan',$id);
        if($this->db->update('tbl_setting_kelurahan',$data)){
            return TRUE;
        }else{
            return FALSE;
        }
    }

    public function delete($id){
        $this->db->where('id_kelurahan',$id);
        if($this->db->delete('tbl_setting_kelurahan')){
            return TRUE;
        }else{
            return FALSE;
        }
    }

    public function whereAnd($data){
        $this->db->where($data);
        return $this->db->get('tbl_setting_kelurahan');
    }

    public function whereOr($data){
        $this->db->or_where($data);
        return $this->db->get('tbl_setting_kelurahan');
    }

    public function lastIDKEC(){
        $this->db->select('id_kelurahan');
        $this->db->order_by('id_kelurahan', 'DESC');
        $this->db->limit(1);
        return $this->db->get('tbl_setting_kelurahan');
    }

    public function viewByKecamatan($id_kecamatan){
        $this->db->where('id_kecamatan', $id_kecamatan);
        $result = $this->db->get('tbl_setting_kelurahan')->result();
        
        return $result; 
    }

}